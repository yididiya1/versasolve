...
const GOLD = 'linear-gradient(135deg,#F0B454 0%,#D98A2B 48%,#C75B39 100%)'
...
function Eyebrow({ label }: { label: string }) {
  return (
    <span className="eyebrow mb-4 reveal">
      <span className="eyebrow-dot" />
      <span className="font-mono text-[10.5px] uppercase tracking-[2.5px] text-gold-dark">{label}</span>
    </span>
  )
}
    ...
    <main className="bg-page text-deep">
      ...
      <section className="relative min-h-[72vh] flex flex-col justify-center overflow-hidden pt-[76px] bg-ink">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div
            ...
            style={{ background: 'radial-gradient(circle, rgba(212,149,58,0.18) 0%, transparent 65%)', filter: 'blur(80px)' }}
          />
          <div
            className="absolute bottom-0 left-1/4 w-[400px] h-[400px] rounded-full"
            style={{ background: 'radial-gradient(circle, rgba(212,149,58,0.08) 0%, transparent 65%)', filter: 'blur(90px)' }}
          />
          <div
        ...
        <div className="relative z-10 px-6 md:px-14 xl:px-24 py-20 max-w-[1400px] mx-auto w-full">
          ...
          <span className="eyebrow mb-5" style={{ borderColor: 'rgba(232,163,61,0.22)', background: 'rgba(232,163,61,0.06)' }}>
            <span className="eyebrow-dot" />
            <span className="font-mono text-[10.5px] uppercase tracking-[2.5px] text-gold">Service 01</span>
          </span>
          ...
          >
            Digital Branding<br />
            <span className="text-gradient-warm">Optimization</span>
          </h1>
          ...
          <div className="flex gap-3 flex-wrap">
            <Link
              href="/#contact"
              className="btn-warm inline-flex items-center gap-2 font-body font-semibold text-[14px] px-7 py-3.5 rounded-full no-underline"
            >
      ...
      <section className="relative overflow-hidden bg-page py-24 px-6 md:px-14 xl:px-24">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto">
          ...
          >
            Three Ways We Build<br />Your <span className="text-gradient-warm">Brand</span>
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {subServices.map((s, i) => (
              <div
                key={s.num}
                className={`reveal reveal-d${i + 1} glass-card card-lift group relative rounded-2xl p-8 overflow-hidden`}
              >
                <div className="absolute top-4 right-5 font-display italic font-bold text-gold/[0.13] select-none pointer-events-none leading-none" style={{ fontSize: '7rem' }}>
                  {s.num}
                </div>
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-5">
                    <span className="grid h-10 w-10 place-items-center rounded-xl text-[17px] text-[#1A140D] shadow-[0_8px_20px_rgba(216,138,43,0.28)]" style={{ background: GOLD }}>
                      {s.symbol}
                    </span>
                    <span className="font-mono text-[10px] tracking-[2.5px] uppercase text-gold-dark/70">{s.num}</span>
                  </div>
                  <h3
                  ...
                  <p className="font-body text-[14.5px] leading-relaxed text-deep-soft mb-6">{s.desc}</p>
      ...
      <section className="relative overflow-hidden bg-page-soft py-24 px-6 md:px-14 xl:px-24">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto">
          ...
          <h2
            ...
            style={{ fontSize: 'clamp(30px, 3.8vw, 50px)' }}
          >
            The Impact of Strong<br />Digital <span className="text-gradient-warm">Branding</span>
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {benefits.map((b, i) => (
              <div
                key={b.title}
                className={`reveal reveal-d${i + 1} glass-card card-lift rounded-2xl p-8`}
              >
                <span className="mb-5 grid h-12 w-12 place-items-center rounded-xl text-[22px] text-[#1A140D] shadow-[0_8px_20px_rgba(216,138,43,0.28)]" style={{ background: GOLD }}>{b.icon}</span>
                <h3 className="font-display italic font-semibold text-[22px] text-deep mb-3 tracking-tight">{b.title}</h3>
                <p className="font-body text-[15px] leading-relaxed text-deep-soft">{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── CTA (light) ──────────────────────────────────────────── */}
      <section className="bg-page py-24 px-6 md:px-14 xl:px-24">
        <div className="max-w-[1400px] mx-auto">
          <div className="reveal glass-card relative rounded-2xl px-8 md:px-16 py-14 md:py-20 text-center overflow-hidden">
            <div
              ...
              style={{ background: 'radial-gradient(ellipse at 50% 50%, rgba(212,149,58,0.14) 0%, transparent 65%)' }}
            />
            <p className="relative font-mono text-[11px] tracking-[3px] uppercase text-gold-dark mb-4">
            ...
            >
              Let&apos;s Build Something<br /><span className="text-gradient-warm">Unforgettable</span>
            </h2>
            ...
            <div className="relative flex gap-3 justify-center flex-wrap">
              <Link
                href="/#contact"
                className="btn-warm inline-flex items-center gap-2 font-body font-semibold text-[14px] px-8 py-3.5 rounded-full no-underline"
              >
                Get Started Today
                <Arrow />
              </Link>
